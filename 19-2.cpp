...
class Map {
  public:
    ...
    int getLargestManhattanDistance() {
      int largest = -1;
      for (const auto & [key, value] : this->offsets) {
        for (const auto & [key2, value2] : this->offsets) {
          if (value == value2) continue;
          largest = max(largest, manhattan(value, value2));
        }
      }
      return largest;
    }
  ...
  public:
    ...
    vector<array<int, 3>> getTransformation(int i) {
      switch (i) {
        ...
        case 23: return transform(2, 0, 1, false,  true,  true);
      }
    }
...
int main(int argc, char **argv) {
  ...
  cout << "Largest manhattan distance: " << overallMap->getLargestManhattanDistance() << endl;
