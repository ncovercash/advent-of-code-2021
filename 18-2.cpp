  ...
  public:
    ...
    Number(Number *copy) {
      if (copy->left != nullptr) {
        this->left = new Number(copy->left);
      }
      if (copy->right != nullptr) {
        this->right = new Number(copy->right);
      }
      this->normalValue = copy->normalValue;
    }
    ...
    void reduce() {
      ...
      if (exploded.first != -1 || exploded.second != -1) {
        cerr << "Exploded into: " << *this << endl;
        this->reduce();
        return;
      }
      if (this->split()) {
        cerr << "Split into: " << *this << endl;
        this->reduce();
        return;
      }
    }
    ...
    static Number* add(Number *a, Number *b) {
      Number *result = new Number(nullptr, new Number(a), new Number(b));
      cerr << "Reducing: " << *result << endl;
      result->reduce();
      return result;
    }
...
int main(int argc, char **argv) {
  ...
  while (!input.eof()) {
    ...
    numbers.push_back(new Number(nullptr, line));
  }

  int highestMagnitude = Number::add(numbers[0], numbers[1])->magnitude();

  for (int i = 0; i < numbers.size(); i++) {
    for (int j = 0; j < numbers.size(); j++) {
      if (i == j) continue;
      highestMagnitude = max(highestMagnitude, Number::add(numbers[i], numbers[j])->magnitude());
    }
  }

  cout << "Highest magnitude: " << highestMagnitude << endl;
